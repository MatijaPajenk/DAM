...
std::vector<Point> load_data(std::ifstream &file, size_t allowed_size) {
...
merge_chunks(const std::string &chunks_dir, const std::string &merged_dir,
  ...
  while (current_level.size() > 1) {
    ...
    for (size_t i = 0; i < current_level.size(); i += 2) {
      ...
      std::ostringstream oss;
      oss << merged_dir << "/chunk_" << merge_iteration << '_' << i / 2
          << ".csv";
  ...
  std::cout << "Merge complete: " << current_level.size() << " leaf files.\n";
...
std::unique_ptr<Node>
build_tree_from_chunks(const std::vector<std::string> &leaf_files,
  ...
  while (level.size() > 1) {
    ...
    for (size_t i = 0; i < level.size(); i += 2) {
      ...
      const double xmax = std::min(L->xmax, R->xmax);
      const double ymin = std::min(L->ymin, R->ymin);
      const double ymax = std::min(L->ymax, R->ymax);
...
std::vector<std::string> query_tree(const Node *root, double xmin, double xmax,
                                    double ymin, double ymax) {
  ...
  std::function<void(const Node *)> traverse = [&](const Node *node) {
    ...
    if (node->xmax < xmin || node->xmin > xmax || node->ymax < ymin ||
        node->ymin > ymax) {
      return;
    }
...
int main() {
  // sort_by_chunks("data/file_mini.csv", "data/sorted");
  // auto leaf_files = merge_chunks("data/sorted", "data/merged");

  // auto root = build_tree_from_chunks(leaf_files, "data/tree");

  auto root = load_tree_index("data/tree");

  double xmin = 2006.776355, xmax = 2070.74366, ymin = 1859.896729,
         ymax = 1971.733619;

  auto candidates = query_tree(root.get(), xmin, xmax, ymin, ymax);
